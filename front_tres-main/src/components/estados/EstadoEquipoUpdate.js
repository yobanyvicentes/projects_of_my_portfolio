...
import { putEstadoEquipos, getEstadoEquipoById } from '../../services/estadoEquipoService';
...
export const EstadoEquipoUpdate = () => {
    ...
    const getEstadoEquipo = async () => {
        try {
            ...
            Swal.showLoading();
            Swal.close();
            const { data } = await getEstadoEquipoById(estadoEquipoId);
    ...
    const handleOnSubmit = async (e) => {
        e.preventDefault();
        const estadoEquipoModel = {
            nombre,
            estado,
        }
        try {
            console.log(estadoEquipoModel);
            Swal.fire({
                allowOutsideClick: false, title: 'Cargando....', text: 'Por favor espere', timer: 2000//milisegundos
            });
            const { data } = await putEstadoEquipos(estadoEquipoId, estadoEquipoModel);
            console.log(data);
            Swal.close();
        } catch (error) {
